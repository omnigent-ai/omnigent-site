...
resolve the secret from a `source`. All three resolve once, at session start, in
the unsandboxed parent process — never inside the sandbox, and never
re-resolved per request (the `databricks_cli` OAuth profile is the one
exception; see below). Whichever kind you use, the resolved value has
surrounding whitespace stripped, and an empty result is a hard error rather
than a silently-disabled credential.

- `{env: VAR}` — reads `VAR` from the **parent process's** own environment
  (the process running the Omnigent server/runner), not from the sandboxed
  agent's environment and independent of `env_passthrough`. A missing or
  empty value raises.
- `{file: /path}` — reads the file's contents as UTF-8. `~` is expanded;
  the path is read on the host filesystem, independent of the sandbox's
  own `read_paths`/`mask_paths` grants. A missing or empty file raises.
- `{command: ...}` — runs the string through a shell (`shell=True`) in the
  parent process, with the parent's environment, and takes trimmed stdout
  as the secret (e.g. `gh auth token`). A 30-second timeout applies; a
  non-zero exit or empty stdout raises, surfacing the command's stderr in
  the error.
...
`file` and `command` sources run in the unsandboxed parent process before the
sandbox starts, and `command` runs through a real shell — so a `source` value
can read a file or execute a command on the host, entirely outside what the
sandbox boundary constrains. Only use them in specs you author or trust;
don't accept `file`/`command` sources from a user-uploaded, shared, or remote
agent spec, since that hands the parent process an attacker-influenced file
read or shell command outside the sandbox boundary.
...
proxy to attach a credential to. For those, add an `env` field to a host-keyed
entry: the proxy mints a random, single-use `oa_cred_*` placeholder and
injects it into that environment variable instead of the real secret. The
client believes it's authenticated and sends the placeholder; the proxy swaps
it for the real value on the way out.
...
(`api.github.com`) via `GH_TOKEN`/`GITHUB_TOKEN`, since `gh` checks those
env vars locally before it will issue a request.

<div className="note">

The credential proxy covers HTTP(S) Bearer and Basic auth only — SSH-based git
remotes are out of scope. Credential-bound hosts also can't use opaque HTTP/2
