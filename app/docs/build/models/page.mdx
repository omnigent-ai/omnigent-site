...
| **Subscription** | A Claude Pro/Max or ChatGPT Plus/Pro plan, via the official `claude` / `codex` CLIs — or **Pi original auth**, which lets the Pi harness use its own login |
...
The **Pi** harness manages its own authentication. Choosing **Pi — original auth**
in `omni setup` tells Omnigent to leave Pi's credentials alone: Pi reads from its
own `~/.pi/agent` as-is, and Omnigent does not manage a provider or write a
`models.json` for it. Unlike the Claude and ChatGPT subscriptions, this option
has no login step — Pi handles its own auth.

Use this when you already sign in to Pi directly and want that login to drive the
`pi` / `pi-native` harness. Any Omnigent-managed credential Pi can consume (an
Anthropic or OpenAI API key, a gateway, or a Databricks workspace) remains an
alternative; the Claude and ChatGPT subscriptions cannot drive Pi, since a CLI
