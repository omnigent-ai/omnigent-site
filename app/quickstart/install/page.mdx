...
Install the CLI, set up your credentials, and launch your first agent.
...
When you install Omnigent on your laptop, the agent runner executes locally, so it has direct access to your files and projects.

The base install covers everyday use. Add extra integrations at install time. Run the same command to upgrade.
...
## Set up or use existing credentials

Omnigent needs credentials to reach an LLM provider. It scans your environment
for existing credentials and offers them as defaults.

```
omni setup
```

You can connect an API key, a provider subscription, a gateway like OpenRouter,
or a Databricks workspace.

See [Models & Credentials](/docs/build/models#credentials) for all supported providers.

## Auto-discover skills, MCPs, and more

Omnigent scans your existing developer environment and picks up what's already there. Skills and MCP configurations carry over automatically. And you can [import session history](/docs/use/coding-agents#import-existing-chats) from Claude Code, Codex, Cursor, and other agents.

## Launch Debby

Try your install with [Debby](/docs/use/builtin-agents/debby), a built-in multi-AI agent that sends every
...
This starts a local server and opens the web UI. Ask something like
`What came first, the chicken or the egg?` and watch the models challenge
each other.
