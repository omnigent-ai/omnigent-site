...
tools:
  docs-api:
    type: mcp
    url: ${DOCS_API_URL}                     # env vars expanded at runtime
    headers:
      Authorization: "Bearer ${API_TOKEN}"   # env vars expanded at runtime
```

Because `url` and `headers` are both expanded, a remote server's config can be
committed to version control with neither the endpoint nor the credential
hardcoded — each user supplies their own at run time.
...
Instead of declaring an MCP server inline under `tools` in `config.yaml`, you
can keep it in its own file under the agent's `tools/mcp/` directory. Omnigent
discovers every `tools/mcp/*.yaml` at load time. A sidecar file names the
server and its transport explicitly (`transport: stdio` with `command`/`args`,
or `transport: http` with `url`) and accepts the same optional `tools`
allow-list as the inline form:
...
The `tools` allow-list is enforced identically whether the server is declared
inline or in a sidecar file: only the listed tool names are advertised to the
model, omitting `tools` exposes everything the server provides, and a `tools`
value that is not a list is rejected at load time with an error naming the
file.
...
- **`url`**: the remote server's endpoint also supports `${...}` expansion (e.g., `url: ${DOCS_API_URL}`), so the endpoint doesn't have to be hardcoded either
...
### Nimble web research and extraction

Two builtins reach the live web through [Nimble](https://www.nimbleway.com/).
`nimble_research` hands a research task to a Nimble Web Search Agent and returns
a cited answer with a confidence signal for each claim. Use `nimble_extract`
when you have a Nimble extract template to run against a page; it returns the
parsed result as structured JSON.

`nimble_research` needs the optional `nimble` extra:

```
pip install 'omnigent[nimble]'
```

Enable them in your agent's `config.yaml`, under `tools.builtins`:
...
Nimble is also a [`web_search`](#websearch) provider for quick lookups that
don't need a research run. For the full config reference (polling, agent
instances, extract templates, and where to get an API key), see
[Nimble's Omnigent connector docs](https://docs.nimbleway.com/integrations/connectors/omnigent).
