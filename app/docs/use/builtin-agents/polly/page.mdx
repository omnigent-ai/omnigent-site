...
| `agy`         | `antigravity-native` | `permission_mode: bypassPermissions` | `--dangerously-skip-permissions` (agy's only pre-emptive control) |
| `kimi`        | `kimi-native`        | `yolo: true`                         | `--yolo` (kimi's auto-approve-tools flag)                    |

The two families of workers set their stance differently:

- **Default-bypass, opt-out.** `codex-native` and `cursor-native` default to
  full bypass because their headless seam has no safe non-bypass default. Set
  `executor.config.yolo: false` on such a worker to keep it prompting — useful
  for a read-only or must-approve sub-agent (its gated calls then surface as
  approval cards). For the Cursor worker, `executor.config.permission_mode: auto`
  (or `exec_mode: auto` / `auto-review`) selects Smart Auto (`--auto-review`)
  instead of full `--yolo`, matching Claude's `permission_mode: auto` ergonomics.
- **Opt-in.** `claude-native`, `antigravity-native`, and `kimi-native` keep
  their normal prompting behavior unless the bundle explicitly declares a bypass
  stance. `antigravity-native` opts in with
  `executor.config.permission_mode: bypassPermissions` (matched exactly — a
  wrong-case or padded value is ignored, and other modes such as `acceptEdits`
  have no agy analogue). `kimi-native` opts in with `executor.config.yolo: true`
  (a real bool or a case-insensitive `"true"` string); note that kimi's `--auto`
  full-autonomy mode is deliberately not mapped, only `--yolo`. An absent or
  unrecognized value leaves the worker prompting.
