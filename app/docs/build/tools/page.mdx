...
Omnigent supports MCP servers, Python functions, sub-agents, inherited tools, and a set of
built-in tools that ship with Omnigent.

## Built-in tools

Built-in tools ship with Omnigent. Declare them under `tools.builtins`, either by name or as
a map when the tool takes config:
...
Search the web from your agent. How it runs depends on the model and the `search_provider`
you name:

- **OpenAI models** — passthrough to OpenAI's native server-side web search. No config
  needed; it uses the LLM API key.
- **Other models** — you must name a `search_provider`. There is no default and no env-var
  fallback, so the engine that runs is always explicit. If `search_provider` is unset (or set
  to an unknown value), `web_search` returns an error listing the options instead of picking
  an engine for you.

| `search_provider` | Credentials | Notes |
| --- | --- | --- |
| `duckduckgo` | None (keyless) | Zero-setup. Best-effort — the public HTML endpoint can rate-limit — so prefer a keyed backend for high-volume search. |
| `google` | `api_key` + `engine_id` | |
| `perplexity` | `api_key` | |
| `nimble` | `api_key` | |
| `tavily` | `api_key` | |

Use the keyless `duckduckgo` backend to search with no credentials:
...
For a sturdier, higher-rate backend, name a keyed provider and pass its credentials:
