...
Some flags apply to the CLI itself rather than to a single command. Place them
**before** the command:

```
omni --profiling run agent.yaml
```

`--profiling` profiles CLI execution, prints a summary, and writes a timestamped
`.prof` file — useful when diagnosing slow startup or command performance. It is
distinct from `omni run --profile <name>`, which selects a Databricks config
profile for a specific run.
