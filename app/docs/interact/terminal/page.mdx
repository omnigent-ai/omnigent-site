...
Native terminal harnesses require `tmux`, so install it first if you plan to use
commands such as `omni claude` or `omni codex`:
...
omni codex                           # Codex
omni goose                           # Goose over ACP
omni hermes                          # Hermes subprocess harness
...
- **Agent-independent.** A session belongs to you, not to a specific agent. Compatible APIs can switch its agent while retaining history, although the web UI does not currently expose this action.
- **Persistent.** Every session has a conversation ID and lives beyond the current terminal window. Native terminal sessions resume with `omni resume <conversation_id>`; other runtimes use `omni run --resume <conversation_id> <agent>`.
