...
- **Runs with** — the agent or harness to run. For native coding agents that
  support it (Claude Code), the dialog also exposes **Model**, **Effort**, and
  **Permission mode** controls; leave each on **Default** to use the agent's
  configured settings. Because automations run unattended, a prompting permission
  mode (**Manual** or **Plan**) will wait for an approval that never comes —
  prefer an auto-running mode.
