...
`Invite member`, per-user `Reset` (password), and `Remove`. Under OIDC these
actions are absent — identities are owned by your IdP and provisioned
automatically on first sign-in, so **Members** is a read-only roster.

Every account with a sign-in session (built-in accounts or OIDC) also gets a
**Settings > Account** section showing who they're signed in as, with a sign-out
control. `Change password` appears in built-in accounts mode only; an OIDC
identity's password lives with the IdP.
