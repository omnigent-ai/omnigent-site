...
Once the `grok` binary is on your `PATH`, Grok Build is seeded into the web
new-chat picker automatically — like the custom ACP agents above, it appears in
the **Harnesses** group (under **More**), not under **Agents**. On a host where
the CLI isn't installed, no Grok row is seeded.
...
Every configured `acp:<slug>` agent is seeded into the web new-chat picker
automatically on any host where it's set up — no `--harness` typing required. It
lands in the **Harnesses** group (under **More**), beside the native CLIs rather
than under **Agents**, because selecting it runs a harness, not a composed
agent. The slug is capitalized for display (so `devin` shows as **Devin**). This
seeding is host-scoped: a shared/remote server that holds no `acp:` config shows
no ACP rows, so you never see a picker entry that can't launch there.
