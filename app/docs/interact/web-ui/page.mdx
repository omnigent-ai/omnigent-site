...
When you start a new session with the Claude Code agent, the new-chat composer
exposes a model and reasoning-effort picker. It defaults to **Opus** with
**Medium** effort — Claude Code's own effective defaults — so you can start a
session without opening the picker. Whatever you select rides along as the
session's model and reasoning effort at launch; you can still switch models
mid-session with the `/model` command.
