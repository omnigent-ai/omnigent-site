...
#### Advertise context and output limits

Gateway models with a large context window (for example a 1M-token GLM) need
their real limits declared, or [Pi](/docs/build/harnesses/supported) falls back
to its own defaults — **128k context / 16k output** — and silently truncates.
Set `context_window` and `max_output_tokens` (both token counts) on the gateway
provider family in `~/.omnigent/config.yaml`:
...
Both fields are optional and must be **positive integers** (a zero or negative
value is rejected at startup). When set, pi-native sessions advertise these
limits in their generated `models.json` so Pi uses the full window instead of
its 128k/16k defaults. When you omit them, Omnigent makes a best-effort match
against its model catalog to fill in the limits, so a well-known model id often
gets the right values automatically.
