...
| Windows  | `windows_jobobject` | Process-tree containment and resource limits |
| Other    | `none`              | No sandboxing (explicit opt-out)             |

On Windows, Omnigent supports the core server, web UI, and SDK-based harness
flows, but the Job Object backend does not provide filesystem or network
isolation. Use Linux or macOS when you need hardened Omnibox boundaries.
...
For HTTP(S) egress, Omnibox can also inject bearer or basic auth at the proxy
for allow-listed hosts. The agent sees a placeholder, while the real credential
stays outside the sandbox and is swapped in only for matching requests.
...
- **Windows Job Object containment.** It contains the process tree, but does not isolate files or network traffic.
