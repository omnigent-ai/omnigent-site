...
<ContentTabs labels={["installer", "uv", "pip", "Homebrew"]}>
  <div>

Pass `--extra` to the installer script (repeat it or comma-separate to add
several):
...
Use the standard `package[extra,...]` syntax:
...
Homebrew can't pass Python extras. Install the base formula with `brew`, then
add extras with `uv` or `pip`.
