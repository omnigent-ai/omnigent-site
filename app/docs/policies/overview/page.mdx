...
export const metadata = pageMeta(
  "Contextual Policies",
  "Policies govern requests, model calls, and tool activity to allow, ask for approval, or deny operations in real time.",
  {
    eyebrow: "Policies",
...
Policies control what your Omnigent can and cannot do. They evaluate requests,
LLM calls, responses, and governed tool activity, including file operations
represented as tool calls, and decide in real time
...
Every policy evaluation receives the current event plus selected session
metadata, such as usage, model, labels, and custom state. Policies do not
automatically receive the complete conversation or tool-call history.
...
Policies are checked in order. `ALLOW` and no opinion continue to later
policies, `ASK` decisions accumulate, and `DENY` stops evaluation immediately.
