...
- Lakebase Postgres backs the Databricks Apps deploy path.
...
The Cloudflare deploy path uses D1 with Cloudflare Containers and R2 for
artifacts. Use that path when you want a scale-to-zero Cloudflare-hosted
server rather than a conventional VM or Postgres-backed container deploy.
