...
- **macOS or Linux** for the installer script. Windows users should install
  directly with `uv`.
- **git**. The installer can install `uv` and provision Python 3.12 for you.
- **tmux** for native terminal harnesses such as Claude Code and Codex.
- **bubblewrap** for native terminal harnesses on Linux.

Node.js is needed only for source builds and some harness CLIs, not the base
wheel.

## Install Omnigent

The base install covers everyday use. Extra integrations are opt-in at install
time. Upgrade any supported installation with `omni upgrade`; rerunning the
installer tab also upgrades an installer-script installation.
...
| Sandbox providers  | `modal`, `daytona`, `blaxel`, `boxlite`, `cwsandbox`, `e2b`, `islo`, `openshell`, `kubernetes` |
| SDK harnesses      | `antigravity`, `copilot`, `cursor`, `agents-sdk`                                               |
| Storage and memory | `s3`, `hindsight`                                                                              |
| Other integrations | `tracing`, `dictation`, `nimble`, `slack`                                                      |
...
- `--extra` — keep or add an extra; repeat the flag to add several.
