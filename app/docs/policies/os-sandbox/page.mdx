...
To disable dotpath masking entirely for a trusted root, use the explicit wildcard
entry `cwd_allow_hidden: ["*"]`. This lets every dotfile and dotdir pass through — useful
for a trusted workspace whose agent-created dotpaths (caches, tool state) must persist
across helper invocations. The escaping-symlink defense stays active even with the
wildcard: a symlink whose target resolves outside the granted roots is still masked. Only
use `["*"]` for roots you trust, since it exposes all hidden files by name.

```
sandbox:
  cwd_allow_hidden: ["*"]                    # allow every dotpath in a trusted root
```
...
| `cwd_allow_hidden`                  | string[] | `[".venv"]`      | Dotfile basenames to allow. `["*"]` allows every dotpath in trusted roots (escaping symlinks still masked)                     |
