...
doing. Projects don't nest, and a session belongs to at most one. Drag sessions
between projects when you reorganize work.
...
Use Settings to manage local preferences and per-harness run-mode memory. If
you switch a native harness into a mode such as planning or agentic execution,
Omnigent remembers that choice for the next session on the same harness.

### Custom agents and MCP

Create a custom agent from the UI instead of hand-writing YAML. The creation
flow covers the prompt, harness, model, and MCP servers. For existing agents,
open **Agent Info** to review and manage MCP servers attached to that session.
...
Click an image to open it in the lightbox when you need a closer look.

## Shortcuts

Use the shortcuts overlay to see the current keyboard shortcuts. Pinned-session
hotkeys let you jump back to important sessions without hunting through the
sidebar.
