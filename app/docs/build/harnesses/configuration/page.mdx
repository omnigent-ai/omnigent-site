...
export const metadata = pageMeta(
  "Configuration",
  "Configure built-in and custom ACP harnesses, authentication, runtime selection, and launch overrides.",
  {
    eyebrow: "Harnesses",
    path: "/docs/build/harnesses/configuration",
  },
);

# Configuration

Choose a harness id from [Supported Harnesses](/docs/build/harnesses/supported),
then configure any authentication or launch settings it needs.
...
1. A `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `GITHUB_TOKEN` environment variable.
2. A token you stored through `omnigent setup` → *configure harnesses* →
   **Copilot**.
3. Your GitHub CLI login — if you've run `gh auth login`, Omnigent asks `gh` for
   the token directly (`gh auth token`), so a logged-in user is ready without
   pasting anything into setup.
...
configure an [`acp:` agent](#custom-acp-agents) whose command passes the `grok`
...
[`acp:` agent](#custom-acp-agents) that declares the passthrough.

## Devin

Devin (Cognition) ships as a builtin harness with the id `devin`. Like Grok
Build, it drives its vendor CLI over the
[Agent Client Protocol](https://agentclientprotocol.com) over stdio — Omnigent
renders its streaming output, reasoning, and tool cards and routes its
permission requests through your [contextual policies](/docs/policies/overview).
It's built in, so you select the `devin` harness id directly instead of
registering a launch command.

```
omni run --harness devin
```

Devin brings **its own auth and model** — Omnigent stores no credential, and a
`/model` pick is refused rather than silently dropped, so it runs its
account-default model. To pin one, configure an [`acp:` agent](#custom-acp-agents)
whose command passes the Devin CLI's own model flag. Install the CLI and log in
through Cognition first:
...
Jcode ([jcode.sh](https://jcode.sh)) ships as a builtin harness with the id
`jcode`. Like Grok Build and Devin, it drives its vendor CLI (`jcode acp`) over
the [Agent Client Protocol](https://agentclientprotocol.com) over stdio —
...
A `/model` pick **is** honored for Hermes without any extra setup (generic ACP
agents need a [curated model list](#curate-the-model-picker) first): the
...
- **Gemini API key** — set a non-empty `GEMINI_API_KEY` in the environment.
  Omnigent selects agy's direct Gemini provider (`modelProvider: "gemini"` in
  the session's isolated agy settings) for you. When `GEMINI_API_KEY` is set it
  takes precedence over any saved OAuth login; clear it to fall back to OAuth.
- **Google OAuth login** — run `agy` once and complete the browser sign-in. The
  `agy` CLI has no `agy login` / `agy auth status` subcommand: the token is
  written on its first interactive run. Omnigent recognizes it at
  `~/.gemini/oauth_creds.json` (macOS), `~/.gemini/antigravity-cli/antigravity-oauth-token`
  (Linux), or the macOS Keychain (agy 1.1.7+).
...
By default a custom ACP agent exposes no model picker in Omnigent — the vendor
CLI owns its own model choice, and any `/model` pick is passed through
best-effort. To offer a curated picker instead, point the agent's spec at a
named provider from the `providers:` block of `~/.omnigent/config.yaml`:
...
The curated list comes from that provider's family `models:` map, for example
`models: {default: model-a, fast: model-b}`. Configure **at least two distinct
model IDs** — tier aliases resolve to concrete IDs, and duplicates collapse, so a
default-only map leaves model switching unrestricted and shows no picker. The
provider's default leads the list; session selections can never add models to
it. Only an *explicitly referenced* provider counts: an unrelated global default
provider does not change a custom ACP agent. The referenced provider must
resolve successfully — a configuration error is reported rather than silently
lifting the restriction.

With curation enabled:

- A model pinned in the spec (`executor.model`) or on the agent's `acp:` entry
  (`model:`) must also appear in the list. An unlisted default prevents the
  session from launching, even when a valid override is selected.
- Model picks are validated up front (in the composer, on `omni run --model`,
  and on the session API), so an ID outside the list is rejected before a turn
  is sent. Clearing a selection restores the approved default.
- Select a model from the session composer and send a turn to apply it through
  ACP without losing the live session, provided the command supports ACP model
  switching. If the switch fails, the turn reports an error and the prompt is
  **not** sent on the previous model; retrying attempts the switch again in the
  same session.

<div className="note">

**Note:** The provider reference supplies **model choices, not credentials**.
The ACP command still owns its gateway URL and authentication, exactly as
before. Configure matching `providers:` definitions on both the server and the
execution host so the picker and the launch agree.

</div>

Some vendor CLIs activate built-in providers on the mere presence of a
credential variable — any value — which lets their own picker bypass the curated
set. Set `OMNIGENT_ACP_ENV_UNSET` on the execution host to a comma-separated
list of environment variable **names** to remove from the ACP command's
environment:

```
export OMNIGENT_ACP_ENV_UNSET=OPENAI_API_KEY,GEMINI_API_KEY
```

The setting propagates through the runner and affects newly spawned commands;
it is unset by default (no scrubbing).
...
To add a runtime that Omnigent does not include, see
[Community Harnesses](/docs/build/harnesses/community).
