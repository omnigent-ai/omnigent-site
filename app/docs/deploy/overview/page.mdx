...
| `OMNIGENT_ACCOUNTS_COOKIE_SECRET` | Built-in accounts cookie secret                 |
| `OMNIGENT_ACCOUNTS_BASE_URL`      | Public server URL for built-in accounts         |
| `OMNIGENT_OIDC_COOKIE_SECRET`     | OIDC session cookie secret                      |
...
**Railway & Render.** On Railway, import the repo, add the Postgres plugin, and
verify its `DATABASE_URL` reference before deploying. Render provisions the app and managed Postgres over
...
dedicated Kubernetes Job whose child Pod runs `omnigent host` and dials back to
the server. Failed host containers restart automatically with bounded retries.
...
Managed hosts support Modal, Daytona, Blaxel, Islo, Boxlite, CoreWeave Sandbox,
E2B, OpenShell, and Kubernetes. See [Cloud Sandbox Host](/docs/deploy/cloud-sandbox-host)
