...
| `intent_gate`                | Records the first user message as the session's intent, then DENYs any tool call that has no plausible connection to that intent. Requires an `llm:` config block; fails open when none is available.      | `classification_prompt` (string)                                                                                                                                                                  |
