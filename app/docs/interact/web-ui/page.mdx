...
Editing an existing task uses the same dialog, including the **Runs with**
picker: you can switch the task to a different agent or harness in place without
recreating it, so the task keeps its id and run history. The picker is prefilled
with the task's current agent. When you switch to a different agent, future runs
use the new one while past runs keep the agent they ran with; per-agent model,
effort, and permission settings don't carry across the switch (they reset to the
new agent's defaults).
