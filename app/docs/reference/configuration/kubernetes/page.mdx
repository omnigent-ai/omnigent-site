...
To shorten the clone step itself, set the provider-independent
[`sandbox.git_clone`](/docs/reference/configuration/cloud-sandbox#git-clone-options)
block (shallow depth, single-branch, blobless filter). The Kubernetes provider
honors it for every repository it clones, including warm-pool Pods.
