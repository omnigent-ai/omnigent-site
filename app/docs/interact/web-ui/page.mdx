...
When a new build of the web UI is deployed, the installed (or open) app detects
it in the background and shows a **A new version of Omnigent is available**
banner with a **Reload** button. Click **Reload** to switch to the new version,
or **Dismiss** to keep working on the current one. Your in-flight session is
never swapped out from under you—the update only applies when you choose to
reload.
