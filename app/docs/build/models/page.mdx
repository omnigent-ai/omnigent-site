...
For [native TUI harnesses](/docs/build/harnesses) the picker lists the harness's
own catalog. OpenCode (`opencode-native`) surfaces its logged-in, refreshed
model list — the same catalog you see in the OpenCode TUI — so bare `/model`
opens a populated picker in the web UI. Picking a model is applied on the
harness's next turn.
