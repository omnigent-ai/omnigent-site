...
export const metadata = pageMeta("Models & Credentials", "Configure models and credentials for Omnigent-managed and vendor-authenticated harnesses.", {
  eyebrow: "Build",
...
Model and credential ownership depends on the harness. Omnigent-managed
harnesses use the model and provider credentials you configure. Own-auth
harnesses such as Devin and Grok Build use the vendor CLI's login and model
selection instead.
...
Use `/model` in a session whose harness supports live model overrides. The agent
keeps its conversation history and tool state; only the model changes. Own-auth
built-in ACP harnesses such as Devin and Grok Build choose their own model and
reject `/model`.
