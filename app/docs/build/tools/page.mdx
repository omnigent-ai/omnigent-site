...
- **`auth:` with `type: databricks`**: resolves an OAuth token from a profile in your `~/.databrickscfg`. Both `type` and `profile` are required — without `type: databricks` the profile is ignored:
