...
### Start a new session with a keyboard shortcut

Press **⌘N** (macOS) or **Ctrl+N** (Windows/Linux) from anywhere in the UI to
jump straight to a fresh new-session composer — the same destination as the
command palette's new-session action. The shortcut is global and
focus-independent: it works even while you're typing in the composer, so you
never have to reach for the mouse to start over.

This shortcut is listed alongside the other global shortcuts (open command
palette, show keyboard shortcuts) in the **Keyboard shortcuts** dialog. It's
disabled when the UI runs embedded in another surface, leaving `Ctrl+N` to the
host page there.
