...
- **Built-in judge** (default) — an LLM call using the server's own `llm:` block.
  No extra infrastructure.
- **External routing API** — the server delegates the decision to an external
  `routes:select` service that you point it at.

Smart routing is **off by default**. Enable it by setting `OMNIGENT_SMART_ROUTING=1`
in the server's environment.

## Enable smart routing

```
OMNIGENT_SMART_ROUTING=1 omni serve
```

With the flag on and no `routing:` block configured, the server uses the built-in
judge backed by your `llm:` block. Users then see an **Auto** option in the harness
picker; picking it defers harness + model selection to the router on the first
message.
