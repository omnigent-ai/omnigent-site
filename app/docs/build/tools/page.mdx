...
Omnigent supports MCP servers, Python functions, built-in tools, sub-agents,
and inherited tools.
...
Omnigent ships a small set of built-in tools for common agent capabilities:

| Tool | What it does |
| --- | --- |
| `web_search` | Search the web from an agent session. |
| `web_fetch` | Fetch a web page or document. |
| `upload_file` / `download_file` | Move files into and out of a session. |
| `list_files` | List files visible to the session. |
| `search_conversations` | Search session history the agent can access. |
| `export_agent` | Export an agent bundle. |

Session-aware agents can also use `sys_session_share` to share a session with
another user when the server grants them permission to do so.
...
You can add MCP servers from the custom agent creation flow, and inspect
session MCP configuration from **Agent Info** in the web UI. YAML stays the
source of truth for agents checked into a repo; the UI is the faster path for
session-scoped or newly-authored agents.
...
Python function tools can run inside a container image. Use `container_image`
for new specs; `docker_image` is still accepted as a legacy alias. Set
`container_runtime: podman` when you want Podman instead of Docker.
