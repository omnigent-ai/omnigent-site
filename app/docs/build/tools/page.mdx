...
Put Python functions under `tools/python/` in the agent bundle. Omnigent
discovers each public function and generates its tool schema from the signature
and type annotations:

```python
# tools/python/summarize_file.py
def summarize_file(path: str) -> str:
    """Summarize a local text file."""
    return open(path).read()
```

TypeScript tools follow the same pattern under `tools/typescript/`.

## Sub-agent tools

Declare agents as tools so a supervisor can delegate work to them. Give each
sub-agent its own directory under the parent's `agents/` folder and list it by
name under `tools.agents`. Omnigent discovers every
...
A sub-agent can have its own [harness](/docs/build/harnesses), model, tools, and
[policies](/docs/policies/overview).

Each child bundle is isolated and sees only the tools it declares. Tool
inheritance and `spec: self` are not supported in `spec_version: 1`.
