...
### Run the host as a background service

`omni host` runs in the foreground and stops when you close the terminal. To
keep your machine registered across logins and reboots, install the host as a
per-user operating-system service:

```
omni host enable                       # local mode
omni host enable --server <server-url> # remote server
```

`enable` installs and starts a per-user service — a **launchd** user agent on
macOS (`ai.omnigent.host`) or a **systemd** user service on Linux
(`omnigent-host.service`) — that runs the host and restarts it on failure. It
prints the service definition path and, on macOS, the log file path. Pass
`--non-interactive` to fail instead of opening browser sign-in while preparing
the service (useful for headless setups); a Databricks-fronted server still
...
`disable` stops and removes the current user's host service. Host services are
