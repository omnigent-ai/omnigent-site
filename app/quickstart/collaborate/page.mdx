...
2. Add Railway's Postgres plugin and wait for its `DATABASE_URL` reference to
   resolve. This deployment is not currently one-click.
3. Deploy the detected Dockerfile. If Railway does not detect the service port,
   set it to `8000`.
...
In the web UI, click **Share** on any session. Grant the teammate **Edit**
access, then copy and send the link. They open it in their browser and join in
real time. Copying the URL alone does not grant access.
