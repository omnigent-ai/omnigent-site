...
Your requirements also apply to later agent-initiated renames through the
`sys_session_rename` tool, not just the first-message title. When custom
requirements are set, the agent's proposed title is reformatted to match them
before the rename lands; if that formatting fails, the current title is kept.
Without custom requirements, the agent's proposal is used as-is, and manual
renames you make yourself are never reformatted.
...
setting. The setting takes effect after the local Omnigent server restarts, for
both new-session titles and agent renames.
