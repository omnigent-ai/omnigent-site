...
You can fork a sub-agent session, which promotes it to a top-level session of
its own. The fork is always a fresh top-level conversation — it appears in the
sidebar and outlives its parent — while the original sub-agent keeps running
under its parent untouched. The promoted session takes the host and working
directory of its parent but does not adopt the sub-agent's own children.
