...
The `Tasks` page (`/tasks`) lists your scheduled tasks — saved prompts that run
an agent on a recurring schedule. Each row shows the task name and a
human-readable summary of its schedule, derived from the stored recurrence rule
(for example `Every day at 9:00 AM`, `Weekdays at 8:00 AM`, or `Hourly at :30`).

For an armed (active) task, the schedule line also carries a relative next-run
label — `Next run in 30m`, `in 15h`, `in 6d`, or `soon` — showing how far away
the next fire is. The instant itself is computed by the server's scheduler (its
authoritative anchor), and the UI only formats how long until then. A paused
task shows a `Paused` pill instead and no next-run label.

Hover a row to reveal its `⋯` action menu:

- **Run now** — trigger an immediate fire of the task without waiting for its
  schedule. It's a manual override that reuses the same run path as the
  scheduler, so it works even on a **paused** task. The run launches in the
  background; the new run appears in the task's run history and updates the
  task's last-run status. If a run for that task is already in flight, Run now
  is skipped rather than launching a duplicate.
- **Edit** — change the task's prompt, schedule, or other settings.
- **Pause** / **Resume** — stop or restart the schedule without deleting the task.
