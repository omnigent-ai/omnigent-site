...
| Sandbox providers  | `modal`, `daytona`, `blaxel`, `boxlite`, `cwsandbox`, `e2b`, `openshell`, `kubernetes` |
| SDK harnesses      | `antigravity`, `copilot`, `cursor`, `agents-sdk`                                       |
| Storage and memory | `postgres`, `s3`, `hindsight`                                                          |

## Homebrew source builds behind a mirror

The Homebrew tap installs a prebuilt bottle by default. If you build from source
on a network that requires internal package mirrors, point the build at them with
...
- `HOMEBREW_PIP_INDEX_URL` routes pip through your mirror, including pip's
  isolated build dependencies.
- `HOMEBREW_CARGO_INDEX_URL` takes a sparse registry index URL ending in `/`,
  without the `sparse+` prefix.

Both overrides are optional and do not affect prebuilt-bottle installs. Replace
the example URLs with your own mirrors.
...
On macOS you can skip the CLI install above: if the `omnigent` CLI isn't
detected, the setup screen installs it for you (via `uv`) before starting a
local server or connecting. See
[Install the CLI from the app](/docs/interact/desktop#install-the-cli-from-the-app-macos).
On other platforms, install the CLI first.
...
Self-upgrade an existing install to the latest release with:

```
omni upgrade
```

Add `--check` to report whether a newer version is available and exit without
changing anything.
...
Building a nightly compiles the web UI, so it needs **git**, plus **Node.js 22**
and **pnpm**, in addition to the base requirements.
...
- `--check` — report the newest nightly and exit non-zero if upgrading would
  change the install, without changing anything.
- `--dry-run` — print the command that would run and exit.
- `--extra` — keep or add extras (comma-separate to add several).
