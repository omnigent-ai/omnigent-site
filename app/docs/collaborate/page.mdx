...
<ul>
  <li>In the <Link href="/docs/interact/web-ui">web UI</Link>, hit <strong>Share</strong> and send the link to your teammate.<ul><li>Share grants one of two permission levels: <strong>VIEW</strong> (read-only, teammates watch the session stream) or <strong>EDIT</strong> (teammates can co-drive and send messages).</li></ul></li>
  <li>From the terminal, run:<pre><code>{"omni attach <session_id>"}</code></pre></li>
</ul>
