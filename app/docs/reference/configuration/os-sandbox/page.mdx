...
- `{env: VAR}` — reads `VAR` from the **parent process's** own environment
  ...
  agent's environment and independent of `env_passthrough`. Missing or
  empty raises.
- `{file: /path}` — reads the file's contents as UTF-8. `~` is expanded;
  the path is read on the host filesystem, before the sandbox's own path
  grants apply. A missing or empty file raises.
...
is effectively parent-side code/file-read execution, not something the
