...
xAI's API is OpenAI-compatible, so Grok models run on the `openai-agents` harness.
Declare them with the `xai/` provider prefix so Omnigent routes them correctly:
...
The prefix is required — a bare `grok-…` model string is treated as an OpenAI
model and won't route to xAI.
