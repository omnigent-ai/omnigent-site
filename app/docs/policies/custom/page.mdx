...
For `request` events, text/code attachments (e.g. a CSV) carry their decoded
content in `text`. Archives, Office documents, and databases (`.zip`, `.docx`,
`.xlsx`, `.pptx`, `.db`, `.sqlite`, `.sqlite3`) are listed with their
`filename` and `content_type` but an empty `text`, so a policy can act on their
names and types without their contents being scanned as text.
...
<span id="reference-policyresponse" />
...
<span id="reference-stateupdateentry" />
