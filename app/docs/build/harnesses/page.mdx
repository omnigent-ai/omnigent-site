...
**Installing the Claude Code CLI:** The `claude-sdk` and `claude-native`
harnesses run Anthropic's `claude` CLI. `omni setup` → *configure harnesses*
installs it for you with Anthropic's native installer (not `npm install -g`), or
...
The native installer writes to a user-writable `~/.local/bin` and self-updates,
so it sidesteps the permission errors an `npm install -g` on a root-owned npm
global prefix can hit. After it's installed, sign in with `claude auth login`
(see [Models & Credentials](/docs/build/models#subscription)).
