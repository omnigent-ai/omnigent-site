...
store. The host forwards the standard harness credential vars — and their
`OMNIGENT_`-prefixed aliases — to its runners:
...
Prefer `OMNIGENT_ANTHROPIC_API_KEY` for Claude Code API-key auth: Omnigent resolves it
into Claude Code's `apiKeyHelper`, so a raw `ANTHROPIC_API_KEY` never appears in the
Claude CLI process. See [Models & Credentials](/docs/build/models) for the full alias
rules.
