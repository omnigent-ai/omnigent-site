...
- The standalone `omnigent-canvas` extension has been removed. Enable the built-in Canvas page with `OMNIGENT_FEATURES=canvas` instead.
- `omnigent update` is deprecated. Use `omnigent upgrade` (or `omni upgrade`) instead.
...
Or install Omnigent for the first time:
