...
A **containerized server reads the same blocks** from its server config file —
`/data/config.yaml` on the Docker volume (or the path in `OMNIGENT_CONFIG`),
the same experience the laptop `-c` flag gives. Set `llm:` (and optionally
`routing:`) there and smart routing is enabled on Docker Compose, cloud, and
Kubernetes deployments too — no code or image change required.
