...
Omnigent supports MCP servers, Python functions, sub-agents, and inherited
tools.
...
Session-aware agents can use `sys_session_share` to share a session with another
user when the server grants them permission to do so.
