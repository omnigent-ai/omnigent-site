...
`omni update` is an alias for `omni upgrade`: it runs the same command with the
same options, so either spelling works. The alias is not listed in
`omnigent --help` to avoid a duplicate entry.
