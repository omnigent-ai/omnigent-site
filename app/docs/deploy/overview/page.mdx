...
In an interactive terminal, `omni host` also opens the server's web UI in your
browser once the host is registered. Pass `--no-open` (or set
`OMNIGENT_HOST_NO_OPEN=1`) to skip that; see
[Suppressing the browser tab](/docs/interact/web-ui#suppressing-the-browser-tab).
