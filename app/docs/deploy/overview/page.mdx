...
**Waking dormant runner Pods (resume).** A Kubernetes managed sandbox can be
resumed in place: when a new message arrives for a session whose runner Pod has
stopped, Omnigent wakes the same sandbox id instead of allocating a new one.
Because a Kubernetes Job cannot be restarted once its host process has exited,
resume deletes the dormant Job and its stale launch-token Secret, then recreates
both under the same sandbox id with a freshly armed token. Operator-managed
PersistentVolumeClaims are external resources and are left untouched, so data on
mounted PVCs (see below) survives the wake; the ephemeral `emptyDir` workspace
is recreated with the new Pod.
