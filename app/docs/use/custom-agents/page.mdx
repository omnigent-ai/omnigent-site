...
Wire in MCP servers, Python functions, or sub-agents. Put local Python tools in
`tools/python/`; Omnigent discovers them from their signatures.
...
Add local functions as Python files under `tools/python/`, for example
`tools/python/repo_search.py`.
