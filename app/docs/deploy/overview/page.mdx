...
On an auth-enabled server, a host registered this way renews its own access in
the background so it keeps running past session-token expiry without a human
re-running `omni login` — see
[Staying logged in](/docs/collaborate/auth#staying-logged-in-hosts--cli).
