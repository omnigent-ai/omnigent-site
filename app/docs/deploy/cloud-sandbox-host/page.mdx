...
**same machine as the server**. Because the guest runs untrusted agent code
next to the server, its default `host` network mode is scoped to an explicit
TCP port allowlist rather than the whole host.

```
sandbox:
  provider: microsandbox
  server_url: http://host.microsandbox.internal:8317   # dial-back path from the guest
  microsandbox:
    host_ports: [8317]                                  # extra host-local ports the guest may reach
```

`host.microsandbox.internal` is the stable guest hostname for the machine
running microsandbox. Under `host` network mode:

- The `server_url` port is added to the host allowlist **only when `server_url`
  targets `host.microsandbox.internal`** (the dial-back path to a locally
  self-hosted server). A public `server_url` needs only public egress and does
  **not** implicitly open the matching port on the host machine.
- List any host-local services the guest legitimately needs in `host_ports`
  (each an integer in `1`–`65535`) — for example `host_ports: [8317]` for a
  local LLM gateway the sandbox env reaches at
  `http://host.microsandbox.internal:8317`. Everything else on the host stays
  unreachable.
...
**OpenShell dormant hosts wake in place:** When a server-managed OpenShell
sandbox goes dormant (stopped while its persistent volume is retained), the
session's host shows as a wakeable "asleep" state rather than a terminal
offline one. When the next message arrives for a session bound to a dormant
OpenShell host, Omnigent resumes the same sandbox in place — keeping its name
and volume — and waits for it to become ready before continuing. In-place
resume requires the OpenShell SDK to expose the resume primitive
(`SandboxClient.start`, added in `openshell` 0.0.105); on an older SDK the host
stays honestly offline instead of advertising a wake that would fail. Upgrade
with `uv pip install --upgrade 'omnigent[openshell]'`. Resume is currently a
cold restart; a warm snapshot restore (dependencies installed and caches warm)
will be advertised once the OpenShell gateway exposes it through the SDK.
