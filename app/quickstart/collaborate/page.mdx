...
1. Go to [railway.app](https://railway.app) and import the [Omnigent repo](https://github.com/omnigent-ai/omnigent) from GitHub, then add a Postgres plugin to the project.
2. Adding the plugin does not wire the database into the app on its own. Open the omnigent service's **Variables**, add a `DATABASE_URL` variable, and set it to a reference to the Postgres service: `${{Postgres.DATABASE_URL}}`. Until this is set, the deploy errors with `DATABASE_URL is required`; add it, then redeploy.
