...
Use this policy to keep an agent scoped to the task it was originally asked to
do. It works in two phases:

1. **Intent capture** — the first user `request` in the session is recorded in
   `session_state` as the authoritative intent. It is immutable once set;
   later messages don't change it.
2. **Intent check** — every `tool_call` is classified against the stored intent
   using the server-level LLM. Tool calls that have no plausible connection to
   the original task are denied before they run, with a reason that quotes the
   original request.

Verdicts (`ON_TASK` / `OFF_TASK`) are cached in `session_state`, keyed by a hash
of the intent, tool name, and serialized arguments, so identical tool calls pay
for only one classifier round-trip. The policy requires a server `llm:` config
and fails open (abstains) when no LLM client is available. It takes no required
parameters.
...
**Best-effort, not a guarantee.** Intent checking is an LLM-based classifier —
a sophisticated adversary can craft inputs that convince it a call is on-task.
Combine it with other policies for defence-in-depth.
