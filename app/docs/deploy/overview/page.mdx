...
| **Hugging Face Spaces** | SQLite (ephemeral unless paid)     | Docker Space                                                                               |
| **Modal**               | Bring-your-own Postgres + Volume   | `modal deploy`                                                                             |
| **Cloudflare**          | D1 + R2                            | Cloudflare Containers                                                                      |
| **Databricks Apps**     | Lakebase Postgres + UC Volumes     | Databricks Asset Bundle                                                                    |
| **Docker / Kubernetes** | Postgres or SQLite, platform-owned | Container image                                                                            |
...
**Cloudflare, Modal, Kubernetes, and Databricks Apps.** These use the same
server/runner split with platform-specific storage. Cloudflare runs the server
on Containers with D1 and R2. Databricks Apps uses Lakebase Postgres and Unity
Catalog Volumes. Kubernetes and other container platforms can run the Docker
image with either Postgres or SQLite, depending on the storage you provide.
...
Islo, E2B, CoreWeave Sandbox, Kubernetes runner pods, NVIDIA OpenShell, and
Boxlite. Check out the [Cloud Sandbox Host](/docs/deploy/cloud-sandbox-host)
