...
effective approach is smarter defaults, automatic routing, and visibility — so
usage can keep growing without cost growing at the same rate.
...
#### Route trivial tasks away from expensive models

Most agent turns are not complex reasoning — they're file reads, short
summaries, grep results, or status checks. `deny_trivial_to_expensive_model`
automatically reclassifies those turns and steers them toward cheaper models,
...
policies:
  smart_routing:
    type: function
    handler: omnigent.policies.builtins.cost.deny_trivial_to_expensive_model
```

Pair this with `cost_budget` and a moderate per-session cap. The routing
policy catches the bulk of wasteful spend; the budget policy backstops the
rare runaway session.
...
at $25, a soft check at $50, and routing to catch trivial tasks. Engineers
can still use any model for complex work — they just get asked before spending
more than $25 in a day.

```yaml
# server config (~/.omnigent/config.yaml or /data/config.yaml)
policies:
  trivial_routing:
    type: function
    function:
      path: omnigent.policies.builtins.cost.deny_trivial_to_expensive_model
  daily_budget:
    type: function
    function:
...
With routing in place, most engineers will never see the spend prompts. The
daily cap exists to surface sessions that are genuinely off the rails, not to
penalize productive use.
