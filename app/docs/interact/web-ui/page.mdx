...
When you start a session, the new-chat composer lets you name a branch to run the
agent in its own git worktree. Naming a new branch reveals a **base branch**
field — the branch the new worktree is cut from. Leave it blank and the worktree
branches off the workspace's current branch.

To avoid retyping the same base every time, set a default under **Settings › Git**.
The **Default base branch** field there (e.g. `main`) is remembered per browser:
when set, the composer auto-fills the base-branch field as soon as you name a new
worktree branch. Leave the setting blank to turn auto-fill off.

The auto-filled value is only a starting point — once you edit the base-branch
field (including clearing it), your choice stands, even if you reopen the branch
dropdown. Clearing the branch name and naming a new one re-arms auto-fill, so the
next branch seeds from the current default again.
